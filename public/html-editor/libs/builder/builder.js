...
/* drag and drop */
	...
	saveElement: function(element, type, name, callback) {
		...
		const reusableToken = localStorage.getItem('omnex_token');
		const reusableHeaders = reusableToken ? {'Authorization': 'Bearer ' + reusableToken} : {};
		fetch(saveReusableUrl, {method: "POST", headers: reusableHeaders, body: new URLSearchParams(data)})
		.then((response) => {
...
Vvveb.Gui = {
	...
	init: function() {
		document.querySelectorAll("[data-vvveb-action]").forEach(function (el,i) {
			...
			// Use wrapper function to allow plugins to override actions after init
			// This is necessary because plugins like plugin-jszip.js load after Gui.init()
			el.addEventListener(on, function(event) {
				const action = el.dataset.vvvebAction;
				if (Vvveb.Gui[action]) {
					return Vvveb.Gui[action].call(this, event);
				}
			});
		});
	...
	deletePage: function(element, e) {
		...
		if (confirm(`Are you sure you want to delete "${page.file}"template?`)) {
			...
			if (detail.page) {
				...
				const deleteToken = localStorage.getItem('omnex_token');
				const deleteHeaders = deleteToken ? {'Authorization': 'Bearer ' + deleteToken} : {};
				fetch(deleteUrl, {method: "POST", headers: deleteHeaders, body: new URLSearchParams({file:page.file})})
				.then((response) => {
					if (!response.ok) {  return Promise.reject(response);  }
					return response.text()
				})
	...
	renamePage: function(element, e, duplicate = false) {
		...
		if (newfile) {
			...
			if (detail.page) {
				...
				const renameToken = localStorage.getItem('omnex_token');
				const renameHeaders = renameToken ? {'Authorization': 'Bearer ' + renameToken} : {};
				fetch(renameUrl, {method: "POST", headers: renameHeaders, body: new URLSearchParams({file:page.file, newfile:newfile, duplicate})})
				.then((response) => {
					if (!response.ok) {  return Promise.reject(response);  }
					return response.text()
				})
