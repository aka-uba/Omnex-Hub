...
## 2026-03-11 - HAL menu visibility for Künye Daðýtým by role/integration
- Request: Keep `Künye Daðýtým` menu always visible for `SuperAdmin`; for company `Admin`/users show it only when HAL integration is active/configured, otherwise hide.
- Changes:
...
- Changes:
  - `locales/tr/common.json`
    - `layout.quickSwitchToTopNav`: `Üst Menü Düzenine Geç`
    - `layout.quickSwitchToSidebar`: `Kenar Menü Düzenine Geç`
- Files:
  - locales/tr/common.json
  - .codex/CHANGE_MEMORY.md
- Checks:
...
- Request:
  - On `/settings/integrations`, make "MQTT Hakkýnda" card text styling consistent with other cards.
...
- Changes:
  ...
  - Added `layout.selectCompany` to 8 locale files:
    - `locales/tr/common.json` = `Firma Seç`
    - `locales/en/common.json` = `Select Company`
    - `locales/de/common.json` = `Unternehmen auswählen`
    - `locales/fr/common.json` = `Sélectionner une entreprise`
    - `locales/nl/common.json` = `Selecteer bedrijf`
    - `locales/ru/common.json` = `???????? ????????`
    - `locales/az/common.json` = `Þirk?ti seçin`
    - `locales/ar/common.json` = `???? ??????`
- Backup:
...
- Request: Verify `/settings/integrations` import API flow with provided API key and `MANAV ÜRÜNLERÝ.xlsx`, ensure dynamic API key appears in docs, confirm company-specific API key behavior, validate auto-import interval behavior, improve changed-file handling flow, and guarantee new companies have import directories.
- Changes:
  - `public/assets/js/pages/settings/IntegrationSettings.js`
...
- Request: Add missing line tool options in template editor from Bölüm 4 (Divider / Line Studio), with backup-first and encoding-safe handling.
- Changes:
